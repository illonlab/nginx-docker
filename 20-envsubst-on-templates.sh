...
auto_envsubst() {
  ...
  local locations_dir="${NGINX_ENVSUBST_LOCATIONS_DIR:-/etc/nginx/locations}"  # Новая переменная
  ...
  local location_suffix="${NGINX_ENVSUBST_LOCATIONS_SUFFIX:-.location-template}"  # Новая переменная
  ...
  if [ ! -w "$locations_dir" ]; then
    entrypoint_log "$ME: ERROR: $template_dir exists, but $locations_dir is not writable"
    return 0
  fi
  ...
  find "$template_dir" -follow -type f -name "*$location_suffix" -print | while read -r template; do
    relative_path="${template#"$template_dir/"}"
    output_path="$locations_dir/${relative_path%"$location_suffix"}"
    subdir=$(dirname "$relative_path")
    mkdir -p "$locations_dir/$subdir"
    entrypoint_log "$ME: Running envsubst on $template to $output_path"
    envsubst "$defined_envs" < "$template" > "$output_path"
  done
